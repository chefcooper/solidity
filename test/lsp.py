...
class JsonRpcProcess:
    ...
    def clear_received_messages(self, num):
        for _ in range(num):
...
class TestParser:
    ...
    def parseRequestAndResponse(self):
        ...
        if self.current_line().startswith(RESPONSE_START):
            ret["response"] = self.current_line()[len(RESPONSE_START)-1:] + "\n"
        ...
        while not self.atEnd():
            ...
            if self.current_line().startswith(BLOCK_END):
        ...
        while True:
            ...
            if user_response == "u":
                self.content = self.content[:testcase.responseBegin] + \
                    prependComments("<- " + self.suite.replace_ranges_with_tags(actual)) + \
...
class SolidityLSPTestSuite: # {{{
    ...
    def expect_equal(self, actual, expected, description="Equality", part=ExpectationFailed.Part.Diagnostics) -> None:
